...
function checkInput() {
        ...
        if(!document.joinIn.mail.value) 
        {
            window.alert("이메일을 입력해주세요.");
            return;
        }
        if(!document.joinIn.id.value) 
        {
            window.alert("아이디를 입력해주세요.");
            return;
        }
        if(!document.joinIn.password.value) 
        {
            window.alert("비밀번호를 입력해주세요.");
            return;
        }
        if(document.joinIn.password.value.length < 8)
        {
            window.alert("비밀번호가 8자 미만입니다.");
            return;
        }
        if(!document.joinIn.password.value != !document.joinIn.checkpw.value) 
        {
